...
module.exports = class Assertions {
    ...
    parse() {
        ...
        if (Util.isString(this._value)
            ...
            if (matchArray.length === 2) {
                throw ParserError.error("Invalid goto - does not have label: " + this._value);
    ...
    validate() {
        ...
        if (!path) {
            throw ParserError.error("Invalid JSON path: " + this._path, undefined, this.interaction);
        }

        if (!OPERATORS.includes(this._operator)) {
            throw ParserError.error("Invalid operator: " + this._operator,
                this.lineNumber,
                this.interaction);
        }
        ...
        if (NUMERIC_OPERATORS.includes(this._operator)) {
            if (isNaN(this.value)) {
                throw ParserError.error("Invalid expected value - must be numeric: " + this.value);
            }
        }
    }
    ...
    get value() {
        ...
        if (Util.isString(this._value)) {
            return Util.cleanString(this._value);
    ...
    toString(json) {
        ...
        return "Expected value at [" + this.path + "] to " + operator + "\n"
            + expectedValueString
            + "Received:\n"
            + "\t" + jsonValue + "\n";
