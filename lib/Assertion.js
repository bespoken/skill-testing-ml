...
module.exports = class Assertions {
    ...
    validate() {
        ...
        if (NUMERIC_OPERATORS.includes(this._operator)) {
            if (isNaN(this.value)) {
                throw HandledError.error(HandledError.CODE_YAML_SYNTAX(), "Invalid expected value - must be numeric: " + this.value);
            }
        }
    }
    ...
    get goto() {
        return StringUtil.clean(this._goto);
    }
    ...
    get value() {
        ...
        if (typeof this._value === "string") {
            this._value = StringUtil.clean(this._value);
        } else if (Array.isArray(this._value)) {
            for (let i = 0;i < this._value.length; i++) {
                this._value[i] = StringUtil.clean(this._value[i]);
            }
        }
        return this._value;
    }
