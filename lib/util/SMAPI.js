...
const FrameworkError = require("./FrameworkError");
const fs = require("fs");
...
module.exports = class SMAPI {
    constructor(token, skillId, stage, locale, askConfigured = false) {
        this.token = token;
...
class Simulation {
    ...
    async get(simulationId) {
        const path = `/v2/skills/${this.smapi.skillId}/stages/${this.smapi.stage}/simulations/${simulationId}`;
    ...
    async post(command, newSession = false) {
        ...
        const path = `/v2/skills/${this.smapi.skillId}/stages/${this.smapi.stage}/simulations`;
        const postOptions = {
            headers: this.smapi.headers(),
            host: "api.amazonalexa.com",
            method: "POST",
