...
class Assertions {
     ...
     */
    validate() {
        const filter = this.interaction ? this.interaction.test.testSuite.filterObject() : undefined;
        if (filter && filter.onValidate) {
            const result = filter.onValidate(this);
            if (result !== undefined) {
                return result;
            }
        }
    ...
    valueAtPath(json) {
        const filter = this.interaction ? this.interaction.test.testSuite.filterObject() : undefined;
        if (filter && filter.onValue) {
            return filter.onValue(this, json);
        }
        try {
