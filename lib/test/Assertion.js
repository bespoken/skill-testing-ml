...
class Assertions {
     ...
     */
    validate() {
        const filter = this.interaction.test.testSuite.filterObject()
        if (filter.assertionValidate) {
            return filter.assertionValidate(this)
        }
    ...
    evaluateString(expectedValue, actualValue, ignoreCase) {
        ...
        if (this._lenientMode) {
            ...
            actualValue = actualValue.replace(toRemoveRegex, "");
    ...
    valueAtPath(json) {
        const filter = this.interaction.test.testSuite.filterObject()
        if (filter.assertionValue) {
            return filter.assertionValue(this, json)
        }
