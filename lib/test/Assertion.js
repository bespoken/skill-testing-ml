...
class Assertions {
     ...
     * @return {boolean}
     */
    evaluate(response, localizedValues) {
        if (this.exit) {
            return true;
        }
        ...
        if (this.operator === "==" || this.operator === "=~") {
            ...
            if (jsonValue !== undefined) {
                if (Array.isArray(localizedValues)) {
                    ...
                    const localizedValue = localizedValues;
                    match = this.evaluateRegexOrString(this.operator, localizedValue, jsonValue, ignoreCase);
                }
            }
            return match;
            ...
            if (Array.isArray(localizedValues)) {
                ...
                for (let localizedValue of localizedValues) {
                    if (ignoreCase) {
                ...
                const valueToCompare = ignoreCase ? localizedValues.toLowerCase() : localizedValues;
