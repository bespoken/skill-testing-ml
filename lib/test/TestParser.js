...
module.exports = class TestParser {
    ...
    parseTest(suite, rawTest, testIndex) {
        ...
        const firstInteraction = rawInteractions[0];
        const firstInteractionKeys = Object.keys(firstInteraction);
        let testMeta = "Test " + testIndex;
        let only = false;
        let skipped = false;
        // If the first element of the first interaction is "test", "test.only", or "test.skip", it is metadata
        if (firstInteractionKeys.length > 0 && ["test", "test.only", "test.skip"].includes(firstInteractionKeys[0])) {
            rawInteractions = rawInteractions.slice(1);
            testMeta = firstInteraction[firstInteractionKeys[0]];
            if (firstInteractionKeys[0] === "test.only") {
                only = true;
            } else if (firstInteractionKeys[0] === "test.skip") {
                skipped = true;
            }
        }
        ...
        for(let i=0; i<rawInteractions.length; i++){
            let rawInteraction = rawInteractions[i];
