...
class TestParser {
    ...
    parseInteraction(test, interactionJSON, index) {
        ...
        if (Util.isObject(interactionJSON)) {
            ...
            if (Array.isArray(elements)) {
                ...
                let operator = "==";
                if (Util.isString(elements) && (elements.startsWith("/") || elements.trim().includes("*"))) {
                    operator = "=~";
                }
    ...
    parseObjectAssertion(interaction, element) {
        ...
        if (Util.isString(value) && (value.trim().startsWith("/") || value.trim().includes("*"))) {
            operator = "=~";
        } else if (Array.isArray(value)) {
            if (value.some(x=> x.trim().startsWith("/") || x.trim().includes("*"))) {
                operator = "=~";
            }
        }
