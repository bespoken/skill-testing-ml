                ...
                testContent += tests[i].interactions
                    .map((interaction) => {
                        ...
                        if (expected.length === 1 
                            ...
                            return `- ${interaction.input} ${expected[0].operator} ${expected[0].value}\n`;
                        }
                        let interactionString = `- ${interaction.input} :\n`;
                        interactionString += expected.map((item) => {
                            if (!Array.isArray(item.value)) {
                                return `  - ${item.action} ${item.operator} ${item.value}\n`;
                            }
                            const value = item.value.map(itemValue => `    - ${itemValue}`).join("\n");
