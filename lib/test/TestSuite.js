...
class TestSuite {
     ...
     */
    get asyncMode() {
        const defaultValue = ["twilio", "sms", "whatsapp"].includes(this.platform)
            ? true
            : false;
    ...
    /**
     * Indicates what is the maximum time the phone number can wait for a reply message
     * @return {number} max wait time for a reply
     */
    get replyTimeout() {
        const defaultValue = ["sms", "whatsapp"].includes(this.platform) ? 10000 : 0;
        return Configuration.instance().value("replyTimeout", this.configuration, defaultValue);
    }

    /**
     * Indicates what is the time in seconds to wait after receiving a message
     * @return {number}
     */
    get replyWaitTime() {
        return Configuration.instance().value("replyWaitTime", this.configuration, 2);
    }
