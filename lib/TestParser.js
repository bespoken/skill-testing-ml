...
module.exports = class TestParser {
        ...
        for (const rawInteraction of rawInteractions) {
            try {
                ...
                if (e.handled) {
                    ...
                    throw ParserError.error(e.message, undefined, test, rawInteraction);
                }
            }
        }
            ...
            if (Array.isArray(elements)) {
                for (const element of elements) {
                    if (_.isObject(element)) {
                        if (element.intent) {
                            interaction.intent = element.intent;
                        } else if (element.slots) {
                            interaction.slots = element.slots;
    ...
    parseStringAssertion(interaction, assertionString) {
        ...
        if (assertionString.indexOf(" ") === -1) {
            throw ParserError.error("Invalid assertion: " + assertionString, undefined, interaction.test, interaction);
        }
