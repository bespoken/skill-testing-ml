...
module.exports = class TestParser {
    ...
    parse() {
        try {
            ...
            if (documents.length > 1 && documents[0].configuration) {
                if (!_.isObject(documents[0].configuration)) {
                    throw HandledError.error(HandledError.CODE_YAML_SYNTAX(),
    ...
    parseTests(rawTests) {
        ...
        for (const test of rawTests) {
            testCount++;
            const parsedTest = this.parseTest(test, testCount);
            if (parsedTest) {
        ...
        for (const rawInteraction of rawInteractions) {
            try {
                ...
                if (e.handled) {
                    ...
                    throw HandledError.error(HandledError.CODE_YAML_SYNTAX(), e.message, testMeta, rawInteraction);
                }
            }
        }
    ...
    parseInteraction(interaction) {
        ...
        if (_.isObject(interaction)) {
            ...
            if (Array.isArray(elements)) {
                ...
                assertions.push(new Assertion("prompt", operator, elements));
            }
        } else {
    ...
    parseStringAssertion(assertionString) {
        ...
        if (assertionString.trim() === "exit") {
            return new Assertion("exit");
        }

        if (assertionString.indexOf(" ") === -1) {
            throw HandledError.error(HandledError.CODE_YAML_SYNTAX(), "Invalid assertion: " + assertionString);
        }
