...
module.exports = class TestRunner {
    ...
    async runSuiteForLocale(testSuite, context, locale) {
        ...
        if (testSuite.bespokenProjectId) {
            await this.saveTestResults(testResults, testSuite, startTimeTestSuite);
        }
    ...
    async executeRunWrapper(batchEnabled, asyncMode, invoker, testSuite, interactions, context) {
        let results = await this.executeRun(batchEnabled, asyncMode, invoker, testSuite, interactions, context);
        if (testSuite.retryOnError && results && results.length > 0
            && results[0].errorOnProcess === "Call was not answered") {
            debug("Retrying execute");
    ...
    async saveTestResults(testResults, testSuite, startTimeTestSuite) {
        ...
        const { bespokenProjectId, fileName, virtualDeviceToken, runId } = testSuite;
        testResults = testResults.map(testResult => testResult.toDTO());
