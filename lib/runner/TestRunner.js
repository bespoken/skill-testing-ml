...
module.exports = class TestRunner {
    ...
    async runSuite(testSuite, context) {
        ...
        if (locales) {
            ...
            let results = await Promise.all(localesList.map((locale) => {
                return this.runSuiteForLocale(testSuite, context, locale);
            }));
    ...
    async runSuiteForLocale(testSuite, context, locale, configurationOverride) {
        // This may have already been called, but needs to be called again inside of the Jest runner
        await Configuration.configure(this._config);
        if (!locale) {
        ...
        const testSuiteWithLocale = this.parseContents(testSuite.fileName, testSuite.rawTestContent, configurationOverride);
        ...
        for (const test of testSuiteWithLocale.tests) {
            ...
            if (test.hasGoto || !(invoker.batchSupported() && batchEnabled)) {
                results = await this.sequentialRun(invoker, testSuiteWithLocale, test.interactions, context, test.hasGoto);
            } else {
    ...
    async sequentialRun(invoker, testSuite, interactions, context, hasGoto) {
        let goto;
        const results = [];
        const asyncMode = testSuite.asyncMode;
        if (asyncMode && hasGoto) {
            // eslint-disable-next-line no-console
            console.log(chalk.yellow("Warning: \"Goto\" functionality is only available when running in sequential mode, set \"batchEnabled\" property to false to enable it"));
        }
    ...
    parseContents(fileName, testContents, configurationOverride) {
        const parser = new TestParser();
        parser.fileName = fileName;
        parser.load(testContents);
        return parser.parse(configurationOverride);
    }
