...
module.exports = class TestRunner {
    ...
    async run(testFile, context) {
        try {
            ...
            return this.runSuite(testSuite, context);
        } catch (e) {
    ...
    async runSuiteForLocale(testSuite, context, locale) {
        ...
        testResults.forEach((e) => {
            ...
            _.set(e, "_testSuite_customerId", customerId);
            _.set(e, "_testSuite_runId", testSuiteWithLocale.runId);
            ...
            _.set(e, "_testSuite_client", testSuiteWithLocale.type);
        });
    ...
    async getCustomerIdFromSource(virtualDeviceToken) {
        ...
        const search = queryString.stringify({
            ...
            "userToken": virtualDeviceToken,
        });
        const endpoint = url.parse(`${baseUrl}/v1/quotaUsage?${search}`);
        ...
        return HTTP.get(({ headers, host, method, path, port, protocol }))
            ...
            .catch((err) => {
                // eslint-disable-next-line no-console
                console.log(chalk.yellow(`Error retrieving customerId(default value will be used): ${err.message}`));
                return "<error getting customer id>";
            });
    }
};
