...
module.exports = class TestRunner {
    ...
    async runSuiteForLocale(testSuite, context, locale) {
        if (!locale) {
            throw Error("Locale must be defined either in the testing.json or the test file itself under the config element");
        }

        const testSuiteWithLocale = this.parseContents(testSuite.fileName, testSuite.rawTestContent);
    ...
    getInvoker(testSuite) {
        ...
        if (type === CONSTANTS.TYPE.e2e) return CONSTANTS.INVOKER.virtualDeviceInvoker;
        else if (type === CONSTANTS.TYPE.unit) {
            if (platform === CONSTANTS.PLATFORM.alexa) return CONSTANTS.INVOKER.virtualAlexaInvoker;
            else if (platform === CONSTANTS.PLATFORM.google) return CONSTANTS.INVOKER.virtualGoogleAssistantInvoker;
        }

        throw Error("valid type and platform must be defined either in the testing.json or the test file itself under the config element");
    }
}
