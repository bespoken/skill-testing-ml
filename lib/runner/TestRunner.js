...
module.exports = class TestRunner {
    ...
    async runSuiteForLocale(testSuite, context, locale) {
        ...
        if (testSuite.projectId) {
            this.saveTestResults(testResults, testSuite, startTimeTestSuite);
        }
    ...
    async saveTestResults(testResults, testSuite, startTimeTestSuite) {
        const urlString = process.env.SOURCE_API_BASE_URL ? process.env.SOURCE_API_BASE_URL : "https://source-api.bespoken.tools";
        const urlParts = url.parse(urlString);
        const token =  process.env.SOURCE_API_ACCESS_TOKEN || "";
        ...
        const { projectId, fileName, virtualDeviceToken } = testSuite;
        const runId = uniqid();
        ...
        if (response.message.statusCode !== 200) {
            throw new FrameworkError("Error saving test results" + " Raw Error: " + response.body.trim());
        }
