    ...
    async runSuiteForLocale(testSuite, context, locale) {
        ...
        for (const test of testSuiteWithLocale.tests) {
            ...
            if (invoker.batchSupported() && batchEnabled && (test.hasGoto || test.hasExit)) {
                ...
                console.log(chalk.yellow("Warning: \"Goto\" and \"Exit\" functionality is only available when running in sequential mode, set \"batchEnabled\" property to false to enable it"));
            }
