...
module.exports = class TestRunner {
    ...
    async runSuite(testSuite, context) {
        ...
        try {
            await invoker.before(testSuite);
        } catch (error) {
        ...
        for (const test of testSuite.tests) {
            const dto = test.toDTO();
            dto.status = "running";
            this.emit("message", undefined, dto, context);
            ...
            dto.interactionResults = results.map(result => result.toDTO());
            dto.status = "done";
            this.emit("result", undefined, dto, context);

            await invoker.afterTest(test);
        }

        await invoker.after(testSuite);
        return testResults;
    }
