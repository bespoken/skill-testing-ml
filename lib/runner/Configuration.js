...
module.exports = class Configuration {
    ...
    static replaceVariablesInsideTestingJson(testingJson) {
        ...
        while (startIndex !== -1) {
            ...
            jsonClone = jsonClone.split("${" + variable + "}").join(process.env[variable]);
    ...
    async loadConfiguration(pathName, cliOverrides) {
        ...
        if (configurationPath) {
            ...
            if (!skillConfigResult.filepath) {
                // eslint-disable-next-line no-console
                console.error("Unable to find the configuration file");
                this.configurationJSON = {};
                return;
            }
        ...
        if (this.configurationJSON.bespokenProjectId) {
            this.configurationJSON.runId = uuid();
        }
    }
