...
module.exports = class Configuration {
    ...
    getValueWithCorrectType(newValue) {
    ...
    value(propertyName, overrides, defaultValue) {
        if (overrides && propertyName in overrides) {
            return this.getValueWithCorrectType(overrides[propertyName].valueOf());
        }

        if (propertyName in this.json()) {
            return this.getValueWithCorrectType(this.json()[propertyName]);
        }
    ...
    overrideConfigurationWithEnvVariables() {
        for (const key of Object.keys(process.env)) {
            ...
            if (value) {
                _.set(this.configurationJSON, key, this.getValueWithCorrectType(process.env[key]));
            }
        }
    }
