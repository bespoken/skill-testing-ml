...
class CLI {
    ...
    async run(argv, configurationOverrides) {
        ...
        const latest_monitoring_modification = (configurationOverrides.client === "monitoring"
            && process.env.LATEST_MONITORING_MODIFICATION
            && !isNaN(new Date(process.env.LATEST_MONITORING_MODIFICATION)))
            ? new Date(process.env.LATEST_MONITORING_MODIFICATION) : undefined;
        ...
        return jestModule.runCLI({ config, runInBand }, [process.cwd()])
            ...
            .then((jestResult) => {
                new ResultsPublisher().publishResults(
                    get(jestResult, "results.testResults", [])
                        .reduce((p, c) => p.concat(get(c, "bespokenResults", {})), [])
                        .map(e => Object.assign({}, e, { latest_monitoring_modification }))
                );
