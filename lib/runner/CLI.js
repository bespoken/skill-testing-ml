...
class CLI {
    ...
    async run(argv, configurationOverrides) {
        ...
        await Configuration.configure(undefined, testPath, configurationOverrides, true);
        ...
        const isRunningE2E = type === CONSTANTS.TYPE.e2e || invoker === CONSTANTS.INVOKER.virtualDeviceInvoker;
        let runInBand = Configuration.instance().value("runInBand");

        if (typeof runInBand === "undefined" || isRunningE2E) {
            runInBand = true;
        }
