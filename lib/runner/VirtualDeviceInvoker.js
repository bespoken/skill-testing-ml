    ...
    async invokeBatch(interactions) {
        ...
        if (messages.length > 0) {
            try {
                ...
                results = parsedError.results.length > 0 ? parsedError.results : results.fill({});
            }
        }
    ...
    parseError(error){
        ...
        if (typeof(error) === "object") {
            if (Util.isErrorObject(error)) {
                ...
                if (error.error) {
                    errorOnProcess = error.error;
                    errorObject.message = errorOnProcess;
                    errorLog = errorOnProcess;
                } else {
                ...
                if (error.error_category) {
                    errorObject.error_category = error.error_category;
                }
                if (error.error_code) {
            ...
            try {
                ...
                if (parsedError.error_category) {
                    errorObject.error_category = parsedError.error_category;
                }
                if (parsedError.error_code) {
                    errorObject.error_code = parsedError.error_code;
                }
        ...
        LoggingErrorHelper.error("bst-test", errorLog);
