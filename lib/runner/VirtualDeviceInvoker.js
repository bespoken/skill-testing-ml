...
module.exports = class VirtualDeviceInvoker extends Invoker {
    ...
    convertUtterance(interaction) {
        ...
        const { launchWords, exitWord, askWord, toWord } = this.getWords(interaction.test.testSuite.locale);
        let utterance = interaction.utterance;
        if (interaction.utterance === "LaunchRequest") {
            utterance = `${launchWords[0]} ${invocationName}`;
        } else if (interaction.utterance.startsWith("AudioPlayer.")) {
            utterance = undefined;
        } else if (interaction.utterance === "SessionEndedRequest") {
            utterance = `${exitWord}`;
        } else {
            const includesInvocationNameRegex = new RegExp(`^(${launchWords.join("|")}) ${invocationName}`, "i");
            ...
            if (interaction.relativeIndex === 0 && !includesInvocationName) {
                utterance = `${askWord} ${invocationName} ${toWord} ${interaction.utterance}`;
            }
        }
        return utterance;
    }
    ...
    getWords(locale){
        ...
        if (locale === "de-DE") {
            // eslint-disable-next-line spellcheck/spell-checker
            launchWords = ["öffne", "starte", "frage"];
            // eslint-disable-next-line spellcheck/spell-checker
            exitWord = "verlassen";
            // eslint-disable-next-line spellcheck/spell-checker
            askWord = "öffne";
            ...
            launchWords = ["open", "ask", "launch", "talk to", "tell"];
            exitWord = "exit";
            askWord = "ask";
            toWord = "to";
        }
