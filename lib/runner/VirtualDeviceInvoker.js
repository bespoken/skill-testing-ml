...
module.exports = class VirtualDeviceInvoker extends Invoker {
    ...
    convertUtterance(interaction) {
        ...
        if (interaction.utterance === "LaunchRequest") {
            ...
            const launchRequestRegex = new RegExp(`^(ask|open|launch|talk to|tell) ${invocationName}`, "i");
            const isLaunchRequest = launchRequestRegex.test(interaction.utterance);
            if (interaction.relativeIndex === 0 && !isLaunchRequest) {
