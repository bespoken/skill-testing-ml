...
module.exports = class VirtualDeviceInvoker extends Invoker {
        ...
        if (messages.length > 0) {
            try {
                ...
                if (parsedError && parsedError.results) {
                    ...
                    errorObject = Object.assign({}, errorObject, this.validateExceptionError(parsedError, error));
                }
            }
        }
    ...
    async sequentialInvocation(interaction) {
        ...
        try {
            results = await this._virtualDevice.batchMessage([message], true);
        } catch (error) {
            const parsedError = this.parseError(error);
            if (parsedError && parsedError.results) {
                results = parsedError.results;
    ...
    async batchAsyncInvocation(interaction, interactions) {
        ...
        try {
            ...
            do {
                ...
                const processedInteractions = virtualDeviceResponse.results;
            ...
            } else if (error && error.results && error.results.length > interactionIndex) {
                rawVirtualDeviceResponse = error.results[interactionIndex];
            }
    ...
    getError(error){
        ...
        if (typeof(objectError) === "string") {
            ...
            if (objectError.error) {
                ...
                } else if (objectError.stack) {
                    return objectError.stack;
                }
    ...
    validateExceptionError(parsedError, exceptionError) {
        if (parsedError) {
            return parsedError;
        }
        ...
        if (exceptionError.stack) {
            return {
                error_category: exceptionError.error_category,
                message: "ERROR - " + (exceptionError.code || exceptionError.message) + " \nRaw message: " + exceptionError.stack,
            };
        }
