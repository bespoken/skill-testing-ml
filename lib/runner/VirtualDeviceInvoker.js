...
class VirtualDeviceResponse extends InvokerResponse {
    ...
    supported (jsonPath) {
        ...
        const displayRegex = /display(\.\w+)*/;
        const rawRegex = /raw(\.\w+)*/;

        const displayMatch = displayRegex.exec(jsonPath);
        const isDisplayPath = displayMatch && displayMatch[0] === jsonPath;

        const rawMatch = rawRegex.exec(jsonPath);
        const isRawPath = rawMatch && rawMatch[0] === jsonPath;
