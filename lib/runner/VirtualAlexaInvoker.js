...
module.exports = class VirtualAlexaInvoker extends Invoker {
    ...
    async invoke(interaction) {
        ...
        if (interaction.utterance === "LaunchRequest") {
            ...
            if (interaction.intent) {
                ...
                const intent = this.detectIntent(interaction.utterance);
                if (intent) {
                    try {
                        response = await this._virtualAlexa.intend(intent.name, intent.slots);
                    } catch (error) {
    ...
    detectIntent(utter) {
        ...
        const intentRegex = /(\w*)((?: \w*=(?:"(?:\s|\w)*"|(?:\w)*))*)/;
        const match = intentRegex.exec(utter);
        if (match && match[0] === utter) {
            const intent = { name: match[1] };
            if (match[2]) {
                const slots = match[2].split(/(\w*=(?:"(?:\s|\w)*"|(?:\w)*))/g);
                intent.slots = slots.filter(notEmpty).map(toSlot);
            }
            return intent;
        }
        return undefined;
