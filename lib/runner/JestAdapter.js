        ...
        if (result.interactionResults.length > 0) {
            for (const interactionResult of result.interactionResults) {
                ...
                const jestResult = asJestResult(result.test,
                    ...
                    interactionResult.interaction, result.locale, result.skipped);
                jestResults.push(jestResult);
            }
        } else {
            const jestResult = asJestResult(result.test, undefined, undefined, undefined, result.test.skip);
            jestResults.push(jestResult);
        }
    }
    return jestResults;
...
function asJestResult(test, errorMessage, interaction, locale, skipped) {
    const errors = [];
    let status = "passed";
